...
function soloMayoresDeEdad(listaDePersonas) {
    //   function criterioFiltrado(e) {
    return e;
   };
//
   return listaDePersona.filter((e) => {
    return e.edad >=18;
   });
}
...
function laPersonaMayor(listaDePersonas) {
    //
       mayor = listaDePersonas[0];
    for(let indice of listaDePersonas)
    {  
        if(mayor["edad"] < indice["edad"])
        {
            mayor = listaDePersonas[indice];
        }
    }
    return mayor;
}
